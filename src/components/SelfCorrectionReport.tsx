 ...
 * 듀얼 메모리 시스템의 가중치 조정 내역을 시각화
 */
import { motion, AnimatePresence } from 'framer-motion';
import { Brain, ArrowRight, TrendingUp, TrendingDown, Zap } from 'lucide-react';
...
export function SelfCorrectionReport({ adjustments, stats, compact = false }: SelfCorrectionReportProps) {
  if (adjustments.length === 0 && stats.newSkills.length === 0) return null;
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="text-xs font-bold text-violet-400">Leaf-Master 자가 수정 보고</span>
      </div>
      ...
      <AnimatePresence>
        {stats.newSkills.map((skill, i) => (
          <motion.div
            ...
            className="flex items-center gap-1.5 text-[11px] text-primary mt-1"
          >
            <Zap className="w-3 h-3" />
            <span>🎯 Leaf-Master가 <strong>'{skill}'</strong>을 획득했습니다</span>
          </motion.div>
        ))}
      </AnimatePresence>

      {/* 진화 스탯 요약 (compact가 아닐 때) */}
      {!compact && (
